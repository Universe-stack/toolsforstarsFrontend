...
export default function Page() {
  const { icon } = useContext(IconsContext);
  const { state, dispatch, handleFetchAllCourses, handleFetchFilteredCourses } = useResource();
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitted(true);
    try {
      await handleFetchFilteredCourses(filter.selectedValue);
    } catch (error) {
  ...
  const handleIconChange = async (selectedIcon) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error('Error fetching courses, try again!');
        throw new Error('Failed to fetch courses');
      }

      if (data.tools.length <= 0) {
        toast.error(`No courses found for ${selectedIcon}`);
      } else {
        toast.success('Courses fetched successfully!');
      }

      setCourses(data.tools);
      setError(null);
    } catch (error) {
  ...
  const fetchData = async (page = 1, retries = 3, delay = 8000) => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      await handleFetchAllCourses(page);
    } catch (error) {
      console.error('Fetching courses failed:', error);
      if (retries > 0) {
        setTimeout(() => fetchData(page, retries - 1, delay * 2), delay);
      } else {
        setError('Fetching courses failed. Please try again.');
      }
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  ...
  const handlePreviousPage = () => {
    if (state.pagination.currentPage > 1) {
      ...
      dispatch({ type: 'SET_PAGINATION', payload: { ...state.pagination, currentPage: previousPage } });
  ...
  const handleNextPage = () => {
    if (state.pagination.currentPage < state.pagination.totalPages) {
      ...
      dispatch({ type: 'SET_PAGINATION', payload: { ...state.pagination, currentPage: nextPage } });
      fetchData(nextPage);
    }
  };
  ...
  const handleFilteredPreviousPage = () => {
    if (pagination.currentPage > 1) {
      handleFetchFilteredCourses(filter.selectedValue, Number(state.pagination.currentPage) - 1);
    }
  };

  const handleFilteredNextPage = () => {
    if (pagination.currentPage < pagination.totalPages) {
      handleFetchFilteredCourses(filter.selectedValue, Number(pagination.currentPage) + 1);
    }
  };
  ...
  const subMit = submitted ? (
    state.courseFiltered?.courses.map((item) => (
  ...
  const content = courses.length > 0 ? (
    courses.map((course) => (
      <ProductCard key={course.id} data={course} />
    ))
  ) : state.courseResources?.courses ? (
    state.courseResources.courses.map((item) => (
      <ProductCard key={item.id} data={item} />
    ))
  ) : (
