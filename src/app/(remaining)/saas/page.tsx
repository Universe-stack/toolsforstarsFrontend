...
const handleFilteredNextPage = ()=>  {
  ...
  const subMit = submitted ? (
    state.saasFiltered?.tools.map((item) => (
      <ProductCard key={item.id} data={item} />
    ))
  ) : null;


  const content = saasTools.length > 0 ? (
    saasTools.map((tool) => (
      <ProductCard key={tool.id} data={tool} />
    ))
  ) : state.saasResources?.tools ? (
    state.saasResources.tools.map((item) => (
      <ProductCard key={item.id} data={item} />
    ))
  ) : (
